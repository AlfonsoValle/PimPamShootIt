    ...
    draw(delta, ctx) {
        ...
        for (let y = 0; y < pacmanMap.length; y++) {
            ...
            for (let x = 0; x < pacmanMap[y].length; x++) {
                ...
                if (mapCharacter == "W") {
                    ctx.rect(x * totalRatio, // x * horizontalSize
                    y * totalRatio, totalRatio, totalRatio);
                }
                if (mapCharacter == ".") {
                    ctx.arc(x * totalRatio + totalRatio / 2, // x * horizontalSize + horizontalSize / 2
                    y * totalRatio + totalRatio / 2, 7, 0, 2 * Math.PI);
                }
                ctx.closePath();
                ctx.fill();
                ctx.stroke();
